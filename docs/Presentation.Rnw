...
\begin{itemize}
  ...
  \item Records which fullfil \texttt{counts <= th} are ``high risk`` and must be swapped across respective hierarchy
  \item \texttt{swaprate} $\tilde$ lower bound for swapped households
...
<<callRS>>=
# call recodSwap()
dat_swapped <- recordSwap(dat,similar,hierarchy,risk,hid,k_anonymity,swaprate)
...
<<benchmark, echo=FALSE,eval=TRUE,fig.cap="Runtime of procedure for different number of households and hierarchy levels.",fig.align="center">>= 
