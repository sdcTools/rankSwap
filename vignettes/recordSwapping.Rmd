...
+ using hierarchy levels `nuts1` to `nuts3` 
...
The data supplied to the function `recordSwap` should idealy be a data.table or data.frame. For more information on the function `recordSwap` one can call the the corresponding help page through
...
The function applied on randomly generated household data with $n=8\cdot 10^4,1.6\cdot 10^5,3.2\cdot 10^5,5\cdot 10^5,10^6$ households.
The parameter `swaprate` was set to 0.05 and the paramter `th` was set 2. In addition the targeted record swapping was applied using 1, 2 and 3 hierarchy levels.
Using 3 hierarchy levels results in 750 distinct geographic areas.
Figure 2 shows the runtime for different levels of $n$ and different number of geographic hierarchies, using the C++ implementation. Using 1 million households and 3 hierarchy levels the runtime is at roughly `r round(mb_all[hier.levels==3&npop==1000000,value])` Seconds. *Note: Benchmarks were run with the script `RUN_benchmark.R` in tests/testthat/*.
